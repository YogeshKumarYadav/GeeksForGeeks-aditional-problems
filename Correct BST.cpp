...
class Solution {
public:
    void findnode(Node *root, Node *&prev, Node *&a, Node *&b) {
        if(!root)
            return;
        findnode(root->left, prev, a, b);
        ...
        prev = root;
        findnode(root->right, prev, a, b);
    }

    void correctBST( struct Node* root ) {
        Node *prev = NULL, *a = NULL, *b = NULL;
        findnode(root, prev, a, b);
        swap(a->data, b->data);
        return;
    }
};
